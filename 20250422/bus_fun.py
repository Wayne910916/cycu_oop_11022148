...
def fetch_bus_stops(route_name):
    """
    使用 Playwright 抓取公車站名與進站時間
    :param route_name: 公車名稱
    :return: 公車站點的詳細資訊列表，包括到達時間、車站序號、車站名稱、車站編號、經緯度
    """
    route_url = f"https://ebus.gov.taipei/thb/StopsOfRoute?nameZh={route_name}"
    with sync_playwright() as p:
        ...
        for stop in stop_elements:
            try:
                # 提取站名、進站時間、車站序號、車站編號、經緯度
                arrival_time_element = stop.query_selector(".auto-list-stationlist-position-time")
